...
- **Decision**: 拡張子 `.pbz` の ZIP ファイルにする（2026-09-24 に JSON から変更）。中身は、ボードと写真の情報を
  まとめた `backup.json` と、写真の本体 `photos/<id>.<jpg|png|webp>`。ZIP は圧縮なし（stored）とし、
  書き込み・読み込みは依存ライブラリなしで `src/backup/zip.ts` に自前で持つ（CRC32 を含めて 150 行ほど）。
  読み込むときは末尾の目次（central directory）を読み、`Blob.slice` で必要な部分だけを取り出す。
  各ファイルの CRC32 を確かめ、合わなければ壊れたファイルとして扱う。
  ファイル選択の `accept` は指定せず、中身が ZIP で `backup.json` の `format` が合うかで判定する。
- **Rationale**: 写真を base64 の文字列にしないので、ファイルが約 25% 小さくなる（25 枚で約 15MB → 約 11MB）。
  さらに、書き出し・読み込みで全体を 1 つの巨大な文字列にしないため、メモリの使用量が大きく減る
  （iOS Safari でタブが落ちるおそれを下げる）。JPEG はほぼ圧縮できないので、圧縮しないことで処理時間もかけない。
  iOS Safari は独自の拡張子を `accept` に書くとファイルを選べなくなることがあるため、拡張子で絞らない。
- **Alternatives considered**: JSON のまま（以前の形式）— base64 で 33% 大きく、メモリも多く使う。
  ZIP を deflate で圧縮 — 写真はほぼ小さくならず、処理時間が増える。fflate などのライブラリ — 圧縮しない ZIP は
  自前で十分書けるので依存を増やさない（憲章 V）。拡張子 `.photobucket` — 長い。`.pbk` — Windows の電話帳ファイルと重なる。
  以前の JSON 形式との互換 — テスト運用中のため持たない（Clarifications 2026-09-24）。
...
## R14. ボードがないときの使い方の案内（FR-028）

- **Decision**: ボード一覧（`#/`）の、ボードがないときの表示に、4 つの手順を番号付きリスト（`<ol>`）の
  文章で置く。手順は「ボードを作る」「マスにやりたいことを書く」「達成したら写真を貼る」
  「全マス達成したら一枚の画像として保存・共有する」。表示するかどうかは、もう読み込んでいる
  ボード一覧が空かどうかだけで決め、「案内を見たか」は保存しない。案内の下には、今と同じ
  「最初のボードを作る」ボタンと「バックアップを読み込む」ボタンを置く。
- **Rationale**: 案内はボードが空のときにだけ出し、もう一度開く入り口も作らない（Clarifications 2026-09-24）。
  そのため新しい状態もデータも要らず、今の空表示の分岐に足すだけで済む。画像やアイコンを使わない
  文章だけの案内なので、依存も読み込むファイルも増えず（憲章 V）、オフラインでもそのまま出る（憲章 III）。
  番号付きリストは、スクリーンリーダーが手順の数と順番を読み上げられる（憲章 IV）。
- **Alternatives considered**: 手順ごとの図やアニメーション — 素材の用意と保守が増える割に、4 つの手順を
  伝えるには文章で足りる。初回だけ出すチュートリアル（オーバーレイ）— 「見たか」を保存する必要があり、
  ボードをすべて消したときに再び出すという決まりと合わない。
