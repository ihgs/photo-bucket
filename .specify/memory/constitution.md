...
- ユーザーが入力したバケットリスト項目と写真は、端末内（IndexedDB 等のブラウザストレージ）にのみ
  保存しなければならない (MUST)。
- 写真やリスト内容（ボードのタイトル、マスの項目、写真、達成状態）を外部サーバーへ送信しては
  ならない (MUST NOT)。広告の選択・配信のために利用してもならない (MUST NOT)。
- 例外として、ボード画面に限り、広告配信サービス（Google AdSense など）と広告の表示に必要な
  通信を行ってよい (MAY)。この通信は以下をすべて満たさなければならない (MUST):
  - ボード画面以外の画面では広告配信サービスと通信しない。
  - 広告の読み込み失敗・ブロック・オフラインが、アプリの他の機能の動作や表示を妨げない。
  - 広告配信サービスが Cookie 等で利用者情報を扱うことをアプリ内のプライバシーポリシーで説明し、
    法令や規約で同意が必要な地域では同意を得るまで広告を読み込まない。
- 上記の広告以外のアナリティクス、トラッキング、外部 API 呼び出しを導入する場合は、
  本憲章の改定を必要とする。
...
広告による収益化は認めるが、外部との通信はボード画面の広告に閉じ込め、
ユーザーの内容は一切外部へ出さないことで信頼の核を守る。
...
- 初回読み込み後は、ネットワークなしでリスト閲覧・編集・写真追加・画像保存の全機能が
  動作しなければならない (MUST)。広告（原則 I の例外）はこの対象外とし、オフライン時は
  表示しなくてよい。広告をオフライン用にキャッシュしてはならない (MUST NOT)。
...
**Version**: 1.2.0 | **Ratified**: 2026-09-22 | **Last Amended**: 2026-09-24
